# DebianNet.pm: a perl module to add entries to the /etc/inet/inetd.conf file
#
...
you rather not do so now ?  Continue?  (n/y) ";
    ...
    # import an external SMF service
    system('/usr/sbin/inetconv >/dev/null 2>&1');
...
sub remove_service {
    ...
    RLOOP: while(<ICREAD>) {
        chomp;
        if (/^$service\b/ or /^$sep$service\b/) {
	    $_ =~ s/$sep//;
            &printv("Removing line: \`$_'\n");
	    my ($name, $sock, $proto) = $_ =~ /(\w+)\s+(\w+)\s+(\w+)\s+.*/;
	    $name =~ s/\//_/g;
	    $proto =~ s/\//_/g;
	    system("/usr/sbin/svccfg delete -f network/*$name*$proto*");
	    system("rm -f /var/svc/manifest/network/*$name-*$proto.xml 2>/dev/null");
        } else {
            print ICWRITE "$_\n";
        }
    }
...
sub disable_service {
    ...
    DLOOP: while(<ICREAD>) {
      ...
      if (/^$service\s+\w+\s+/ and /$pattern/) {
          $line =~ s/^(.+)$/$sep$1/;
	  my ($name, $sock, $proto) = $1 =~ /(\w+)\s+(\w+)\s+(\w+)\s+.*/;
	  $name =~ s/\//_/g;
	  $proto =~ s/\//_/g;
          &printv("Processing service \`$name-$proto' ... disabled\n");
	  system("/usr/sbin/inetadm -d network/*$name*$proto* >/dev/null 2>&1");
      }
      print ICWRITE "$line\n";
    }
    close(ICREAD);
...
sub enable_service {
    ...
    while(<ICREAD>) {
      ...
      if (/^$sep$service\s+\w+\s+/ and /$pattern/) {
          $line =~ s/^$sep//;
	  my ($name, $sock, $proto) = $line =~ /(\w+)\s+(\w+)\s+(\w+)\s+.*/;
	  $name =~ s/\//_/g;
	  $proto =~ s/\//_/g;
          &printv("Processing service \`$name-$proto' ... enabled\n");
	  system("/usr/sbin/inetadm -e network/*$name*$proto* >/dev/null 2>&1");
      }
      print ICWRITE "$line\n";
    }
    close(ICREAD);

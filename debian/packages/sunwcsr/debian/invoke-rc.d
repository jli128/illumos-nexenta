...
smfed () {
    manifest=""
    manifest_fmri=""
    if test -x "/lib/svc/method/${INITSCRIPTID}"; then
        manifest=$("/lib/svc/method/${INITSCRIPTID}" manifest)
        if test ! -f $manifest; then
            printerror "$manifest: file does not exist"
            exit 1
        fi
        ...
        found_manifest=$(/usr/sbin/svccfg -s system/manifest-import listprop general/${INITSCRIPTID}_manifest|awk '{print $3}')
        if test "x$found_manifest" != x; then
            if test ! -f $found_manifest; then
                printerror "system/manifest-import@general/${INITSCRIPTID}_manifest: property not found"
                exit 1
            fi
            manifest=$found_manifest
        fi
	manifest_fmri=$(echo $manifest|perl -pe "s;/var/svc/manifest/([\w\/]+)/(\w+)\.xml;svc:\/\1/\2:default;")
        return 0
    fi
    return 1
}
...
if test x${FORCE} != x || test ${RC} -eq 104 ; then
	...
	while test ! -z "${ACTION}" ; do
		...
		printerror executing initscript action \"${saction}\"...
            ...
            if smfed; then
                if test ! -r /etc/svc/volatile/repository_door; then
                    printerror "${INITSCRIPTID}: service state unchanged."
		    exit 0
		fi
                case ${saction} in
                    start) saction="enable -st" ;;
                    stop|force-stop) saction="disable -st" ;;
		    reload|force-reload) saction="refresh" ;;
                esac
		if test "x${saction}" = xstatus; then
		    /usr/bin/svcs -l "${manifest_fmri}" && exit 0
	            RC=$?
		elif test "x${saction}" = xrestart; then
                    /usr/sbin/svcadm disable -t "${manifest_fmri}"
                    /usr/sbin/svcadm enable -t "${manifest_fmri}" && exit 0
                else
                    /usr/sbin/svcadm ${saction} "${manifest_fmri}" && exit 0
	            RC=$?
                fi
            else	

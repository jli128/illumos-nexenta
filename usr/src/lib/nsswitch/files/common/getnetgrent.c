	...
	void			*a;
{
	...
	get_be = (struct files_getnetgr_be *)malloc(sizeof (*get_be));
	if (get_be == 0) {
		return (NSS_UNAVAIL);
	}
...
match_triple(ccp triple[NSS_NETGR_N], void *ia_arg, nss_status_t *return_val)
{
	...
	if (__nss_files_netgr_debug != NULL)
		__nss_files_netgr_debug("match_tripple: found");
...
/*ARGSUSED*/
nss_backend_t *
_nss_files_netgroup_constr(dummy1, dummy2, dummy3)

 ...
 */
static int
bisearch(wchar_t wc, const struct wcinterval *wctable, int max)
{
	int	min = 0;
	int	mid;

	if (wc < wctable[0].first || wc > wctable[max].last)
		return (0);
	while (max >= min) {
		mid = (min + max) / 2;
		if (wc > wctable[mid].last)
			min = mid + 1;
		else if (wc < wctable[mid].first)
			max = mid - 1;
		else
			return (1);
	}
...
wcwidth(wchar_t wc)
{
	 ...
	 * non-spacing characters generated using
	 * "uniset +cat=Me +cat=Mn +cat=Cf -00AD +1160-11FF +200B c"
	 */
	static const struct wcinterval combining[] = {
	...
	if (bisearch(wc, combining,
	    sizeof (combining) / sizeof (struct wcinterval) - 1))
		return (0);

	/*
	 * If we arrive here, wc is not a combining or

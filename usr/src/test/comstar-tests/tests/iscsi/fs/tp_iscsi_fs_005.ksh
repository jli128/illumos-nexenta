...
#
# DESCRIPTION:
#	Running I/O data validation on multi initiators session to one target
#
# STRATEGY:
#	Setup:
#		MPXIO is enable on initiator host
#		zfs LUs (specified by VOL_MAX variable in configuration) are
...
#		    to create multi (specified by SESSION_MAX variable in 
#		    configuration) sessions individually to support mutlti-pathing 
...
# __stc_assertion_end
#
function iscsi_fs_005
{
	cti_pass

        tc_id="iscsi_fs_005"
	tc_desc="verify I/O data validation on multi initiators session to one target"
	...
	iscsiadm_modify POS $ISCSI_IHOST initiator-node -c $SESSION_MAX
	...
	[[ $ret -eq 0 ]] && cti_pass "tp_iscsi_fs_005: PASS"
}

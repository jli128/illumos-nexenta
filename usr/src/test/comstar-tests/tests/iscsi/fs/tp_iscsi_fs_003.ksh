...
#
# DESCRIPTION:
#	Running I/O data validation on BLOCK DEVICE backing store
#
# STRATEGY:
#	Setup:
#		MPXIO is enable on initiator host
#		Block device LUs are created in iSCSI target host 
...
# __stc_assertion_end
#
function iscsi_fs_003
{
	cti_pass

        tc_id="iscsi_fs_003"
	tc_desc="verify I/O data validation can pass BLOCK DEVICE backing store"
	...
	set -A bdevs $BDEVS
	typeset index=0
	while [ $index -lt ${#bdevs[@]} ]
	do
		sbdadm_create_lu POS -s ${VOL_SIZE} ${bdevs[$index]}
		(( index+=1 ))
	done
	...
	[[ $ret -eq 0 ]] && cti_pass "tp_iscsi_fs_003: PASS"
}

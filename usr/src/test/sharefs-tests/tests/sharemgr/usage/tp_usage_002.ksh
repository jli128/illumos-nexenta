...
#
# ident	"@(#)tp_usage_002.ksh	1.4	09/08/01 SMI"
#
...
#DESCRIPTION:
#
#	Check individual usage doesn't present incorrect data when a 
#	bad option is given.
#
#STRATEGY:
#
...
#		- Check all the documented options are listed.
#       Cleanup:
#               - N/A
#
#       STRATEGY_NOTES:
...
function usage002 {
	tet_result PASS
	tc_id="usage002"
	tc_desc="Check the usage output of sharemgr sub-commands with bad option"
	...
	do
		result=0
		$SHAREMGR $usage_item $badopt 2>&1 | grep usage > $usage_log 2>&1
		append_cmd "$SHAREMGR $usage_item $badopt"
		if [ "$report_only" ]
		then
			continue
		fi
		...
		if [ $result -ne 0 ]
		then
			tet_infoline "shown_item = $shown_item"

...
#
# ident	"@(#)tp_remove_009.ksh	1.3	08/06/11 SMI"
#
...
#DESCRIPTION:
#
#	Attempt to remove a valid share from a non-associated share group.
#
#STRATEGY:
#
#	Setup:
...
#	Test:
#		- Attempt to remove the valid share (the one added to the
#		  first share group) but specify the wrong share group (second
#		  group).  The operation should fail.
#		- Verify (by new and legacy* methods) that the share is still
#		  in the first group is still shared and has the default
#		  options.
#	Cleanup:
...
function remove009 {
	tet_result PASS
	tc_id="remove009"
	tc_desc="Attempt a remove a valid share from a non-associated group"
	cmd_list=""
	unset GROUPS
	...
	#
	remove_share NEG test_group_2 "-n" ${MP[0]}
	remove_share NEG test_group_2 "" ${MP[0]}
	#
	# Cleanup
	#

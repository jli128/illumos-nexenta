...
#
# ident	"@(#)tp_remove_007.ksh	1.3	08/06/11 SMI"
#
...
#DESCRIPTION:
#
#	Attempt to remove a share without specifying a target group.
#
#STRATEGY:
#
#	Setup:
...
#	Test:
#		- Attempt to remove a share but do not provide a target group
#		  name.  The operation should succeed.
#		- Verify (by new and legacy* methods) that the share in the
#		  first group is no longer shared.
#	Cleanup:
...
function remove007 {
	tet_result PASS
	tc_id="remove007"
	tc_desc="Attempt a remove a valid share without specifying target group"
	cmd_list=""
	unset GROUPS
	...
	#
	remove_share POS "" "-n" ${MP[0]}
	remove_share POS "" "" ${MP[0]}
	#
	# Cleanup.
	#

...
#
# ident	"@(#)tp_disable_010.ksh	1.3	08/06/11 SMI"
#
...
#DESCRIPTION:
#
#	Disable/enable non-existent group
#
#STRATEGY:
#
#	Setup:
#		- Create control share group with default properties.
#		- Populate second group with one share.
#	Test:
#		- Disable non-existent group name.
#		- Verify that control group is still enabled.
#		- Enable non-existent group name.
...
function disable010 {
	tet_result PASS
	tc_id="disable010"
	tc_desc="Disable/enable non-existant group"
	cmd_list=""
	unset GROUPS
	...
	#
	# Disable bogus test group.  (Dry run first then the real thing.)
	disable NEG "-n" bogus_test_group
	disable NEG "" bogus_test_group

	# Verify that ${TG[1]} is still enabled
	verify_group_state ${TG[1]}

	# Enable bogus test group.  (Dry run first then the real thing.)
	enable NEG "-n" bogus_test_group
	enable NEG "" bogus_test_group

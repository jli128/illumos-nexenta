...
#
# ident	"@(#)tp_delete_001.ksh	1.2	08/12/19 SMI"
#
...
#
# ASSERTION: delete_001
#
# DESCRIPTION:
#	Delete a lofi device based on the name of the file.
#
# STRATEGY:
#	Setup
...
#	Assert
#		- Delete the lofi device by specifying the file name to
#		  'lofiadm -d'
#	Cleanup
#		- Remove the file
#
...
function tp_delete_001 {
	...
	# Initialization
	cti_pass
	cti_assert $ASSERTION "Delete lofi device by file name"
	...
	if (( $status == 0 )); then
		...
		lofi_add_status=$?
		if (( $lofi_add_status != 0 )) && (( $lofi_add_status != 2 )); then
			cti_unresolved "Unable to test lofi deletion because" \
			    "creation failed"
			status=1
		fi
	...
	# Assertion -- delete lofi device using name of file
	if (( $status == 0 )); then
